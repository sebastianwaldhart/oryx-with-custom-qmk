...
bool stop_dead_key(bool activated, void* ctxt) {
    if (!activated) SEND_STRING(SS_TAP(X_SPACE));
    return true;
}

#define make_td_function(NAME, KEYS) \
string (NAME)_keys[] = { (KEYS) }  \
void (NAME)(tap_dance_state_t *state, void* user_data) { \
    if (state->count < (sizeof((NAME)_keys) / sizeof(uint16_t))) { \
        SEND_STRING(SS_TAP((NAME)_keys[state->count])); \
    } else { \
        SEND_STRING(SS_TAP((NAME)_keys[0])); \
        while (--state->count > 0) { \
            SEND_STRING(SS_DELAY(20) SS_TAP((NAME)_keys[0])); \
        } \
    } \
}

#define create_override_dead_key(T, IT, NT, K, X) {\
  .trigger_mods                          = (T), \
  .layers                                = ~(1 << _LAYER6), \
  .suppressed_mods                       = (IT), \
  .options                               = ko_options_default, \
  .negative_mod_mask                     = (NT), \
  .custom_action                         = &stop_dead_key, \
  .trigger                               = (K), \
  .replacement                           = (X), \
  .context                               = NULL, \
  .enabled                               = NULL, \
...
const key_override_t override_shift_6 = create_override_dead_key(MOD_MASK_SHIFT, MOD_MASK_SHIFT, MOD_IGNORE_ALWAYS, KC_6, KC_GRAVE);
const key_override_t override_circ = create_override_dead_key(0, 0, 0, KC_CIRC, KC_GRAVE);
...
const key_override_t override_tilde = create_override(MOD_MASK_SHIFT, MOD_MASK_SHIFT, MOD_IGNORE_ALWAYS, KC_GRAVE, RALT(KC_RBRC));
const key_override_t override_grave = create_override_dead_key(0, 0, MOD_IGNORE_ALWAYS, KC_GRAVE, RSFT(KC_EQUAL));
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_LAYER6] = LAYOUT_planck_grid(
    DE_CIRC,        KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, DE_EURO,        KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT,
...
const uint8_t PROGMEM ledmap[][RGB_MATRIX_LED_COUNT][3] = {
    ...
    [6] = { {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {85,203,158}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {134,255,213}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {0,0,255}, {134,255,213}, {134,255,213}, {134,255,213}, {134,255,213}, {85,203,158}, {85,203,158}, {134,255,213}, {85,203,158}, {85,203,158}, {134,255,213}, {0,0,255}, {134,255,213} },
